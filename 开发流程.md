...
DevTools 是一个基于 Vue 3 和 Element Plus 的在线工具集合，提供了丰富的编程辅助工具。本文档将指导你如何在 DevTools 平台上开发和集成一个新工具。
...
<template>
  <el-dialog
    ...
    <Base64Converter v-else-if="store.currentTool?.id === 'base64'" :key="`base64-${store.currentTool.id}`" />
    <JsonFormatter v-else-if="store.currentTool?.id === 'json'" :key="`json-${store.currentTool.id}`" />
    <!-- 你的新工具组件 -->
    <YourTool v-else-if="store.currentTool?.id === 'your-tool-id'" :key="`your-tool-${store.currentTool.id}`" />
    <div v-else>
      <p>工具内容正在开发中...</p>
    </div>
  </el-dialog>
