...
const highlightedText = computed(() => {
  ...
  try {
    ...
    return testText.value.replace(regexObj, (match) => `<mark class="highlight">${match}</mark>`)
  } catch (error) {
...
<template>
  <div class="regex-tester">
    <div class="tool-header">
      <el-button type="primary" link @click="showCommonPatterns = !showCommonPatterns">
        {{ showCommonPatterns ? '隐藏常用正则' : '显示常用正则' }}
      </el-button>
    </div>

    <!-- 常用正则表达式模板 -->
    <div v-if="showCommonPatterns" class="common-patterns">
      <el-tabs type="border-card">
        <el-tab-pane
          v-for="category in commonPatterns"
          :key="category.category"
          :label="category.category">
          <el-table :data="category.patterns" style="width: 100%" size="small">
            <el-table-column prop="name" label="名称" width="180" />
            <el-table-column prop="pattern" label="表达式" />
        ...
        <el-form-item label="正则表达式">
          <div class="regex-with-options">
            ...
            <div class="regex-options">
              <!-- 修改：使用独立的复选框，并且正确绑定值 -->
              <el-checkbox v-model="hasGlobalFlag" border> 全局匹配 (g) </el-checkbox>
              <el-checkbox v-model="hasIgnoreCaseFlag" border> 忽略大小写 (i) </el-checkbox>
              <el-checkbox v-model="hasMultilineFlag" border> 多行匹配 (m) </el-checkbox>
            </div>
          </div>
        </el-form-item>
      </div>
    ...
    <el-row :gutter="20">
      ...
      <el-col :span="12">
        <div class="input-output-section">
          ...
          <!-- 移动替换结果到这里 -->
          <div v-if="showReplaceResult" class="replace-result">
      ...
      <el-col :span="12">
        <div class="results-section">
          ...
          <el-card class="result-card" shadow="never">
            <div v-if="regex && testText" v-html="highlightedText" class="highlighted-text"></div>
    ...
    <div class="regex-help">
      <el-collapse>
        <el-collapse-item title="正则表达式基础" name="1">
          <div class="help-content">
            <p><b>元字符：</b> ^ $ . * + ? | \ ( ) [ ] { }</p>
            <p><b>字符类：</b> [abc] 匹配'a'或'b'或'c'， [^abc] 匹配除了'a'、'b'、'c'之外的字符</p>
            <p>
              <b>预定义类：</b> \d 数字， \w 字母数字下划线， \s 空白符， \D 非数字， \W
              非字母数字下划线， \S 非空白符
            </p>
            <p>
              <b>重复：</b> * 零或多次， + 一或多次， ? 零或一次， {n} 恰好n次， {n,} 至少n次，
              {n,m} n到m次
            </p>
            <p><b>捕获组：</b> (pattern) 捕获匹配的子串</p>
            <p class="code-example">
              例如: <code>(\d{3})-(\d{4})</code> 能匹配 "123-4567" 并捕获 "123" 和 "4567"
              作为组1和组2
            </p>
            <p><b>非捕获组：</b> (?:pattern) 匹配但不捕获</p>
            <p><b>命名捕获组：</b> (?&lt;name&gt;pattern) 给捕获组起名，可以通过名称引用</p>
            <p class="code-example">
              例如:
        ...
        <el-collapse-item title="常用正则表达式技巧" name="2">
          <div class="help-content">
            <p><b>转义：</b> 特殊字符前需加反斜杠\来转义，例如 \. 匹配点号</p>
            <p><b>贪婪与懒惰：</b> *?, +?, ?? 懒惰匹配（尽可能少的匹配字符）</p>
            <p class="code-example">
              例如: <code>&lt;.+&gt;</code> 贪婪匹配 "&lt;div&gt;content&lt;/div&gt;" 整体，而
            ...
            <p class="code-example">例如: <code>\w+(?=@)</code> 匹配邮箱地址中@之前的用户名</p>
            <p><b>条件匹配：</b> (?(条件)是|否) 根据条件匹配不同的模式</p>
            <p><b>常见匹配模式：</b></p>
            <ul>
              <li>电话号码: <code>\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}</code></li>
              <li>
                强密码:
        ...
        <el-collapse-item title="实用示例" name="3">
          <div class="help-content">
            <p>
              <b>匹配HTML标签：</b>
              <code>&lt;([a-z]+)(?:\s+[a-z]+=".*?")*\s*&gt;.*?&lt;\/\1&gt;</code>
            </p>
